# Factory Governed-Layers Build Prompt — v1.1
...
- **1.1** Add `tenant_id` to every corpus / document / chunk / embedding / formula /
  procedure table (Alembic migration; ships WITH the code, never hand-applied).
- **1.2** Postgres **Row-Level Security** on every one of those tables, keyed by
  `tenant_id`, policy bound to a session variable (e.g. `app.tenant_id`) set at
  the start of every request. The app's DB role must have NO bypass.
- **1.3** Per-tenant vector namespace. **State the vector store explicitly.**
  - If **pgvector**: the embeddings table sits under the same RLS policies and
    T1.1 must cover it.
  - If **external** (Pinecone / Qdrant / equivalent): the isolation claim for the
    vector surface is **app-level**, not structural — say so plainly in the phase
    report. Do not call it structural.
- **1.4** Keep existing app-level project/tenant filtering as defense-in-depth. It
  is never the only guard.
- **1.5** Caches and embedding lookups keyed by tenant. The prompt-assembly path
  takes exactly one tenant.
- **1.6** Admin/ops tooling reads through the same RLS'd session or a separate
  audited service — never a role that can see all tenants from the app.
- **1.7** The RLS session variable MUST be **`SET LOCAL` inside each
  transaction**. `SET SESSION` does not survive transaction-mode pooling. **Verify
  the PgBouncer pooling mode on Render and report it** in the phase report.
- **1.8** The app DB role must NOT be a table owner and must NOT have `BYPASSRLS`.
  Add a **startup assertion that refuses to boot** (named reason) if the
  connecting role has `BYPASSRLS`. Silent RLS-off is the zero-artifact bug in
  database form.

Acceptance tests:

- **T1.1** `test_cross_tenant_read_returns_zero`: from tenant A's session, query
  tenant B's documents, chunks, vectors (incl. the embeddings table if pgvector),
  formulas, procedures → ZERO rows on every table.
- **T1.2** `test_cross_tenant_retrieval_never_leaks`: run the full retrieval
  entrypoint as tenant A against a query only tenant B's corpus can answer → no
  B chunk in the result, and the answer refuses with `tenant_isolation_no_match`
  (never B's content).
- **T1.3** `test_admin_role_cannot_bypass_rls`: with the tenant session var
  unset, the app role sees ZERO rows (never all rows).
- **T1.4** `test_app_role_has_no_bypassrls`: boot with a `BYPASSRLS` role →
  refuses to start with a named reason.

Mutation probe: **P1** `probe_tenant_isolation` — disable the RLS policy (or
unset the session var) and assert T1.1 / T1.2 FAIL. If disabling isolation does
not turn the suite red, the phase is REJECTED.
...
- **4.1** The certified kit **contains the retrieval engine**: a real embedding
  model, a vector store (pgvector or equivalent, tenant-partitioned per Phase 1),
...
- **5.4** Multi-tenant concurrency: each job gets its own checkout + branch; the
  worker runs under the **tenant-scoped DB role** (Phase 1 isolation applies to
