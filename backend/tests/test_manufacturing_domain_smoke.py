...
It patches the chain generator to return a deterministic manufacturing chain and then
...
# Deterministic LLM response for the manufacturing analysis concept.
MANUFACTURING_CHAIN: Dict[str, Any] = {
    ...
    "rules": [
        "Always flag equipment downtime",
        "Always flag quality control deviations",
        "Always flag supply chain delays",
        "Always flag safety compliance issues",
    ],
}
...
def fake_manufacturing_engine(tmp_path: Path, monkeypatch) -> Path:
    ...
    kit_root = engine_root / "block_store" / "kits" / "manufacturing"
    bundle = kit_root / "bundle"

    # Kit artifacts
    (bundle / "app" / "blocks").mkdir(parents=True)
    (bundle / "app" / "blocks" / "manufacturing_v2.py").write_text(
        "class ManufacturingBlockV2:\n    name = 'manufacturing_v2'\n",
        encoding="utf-8",
    )
    (bundle / "app" / "containers").mkdir(parents=True)
    (bundle / "app" / "containers" / "manufacturing.py").write_text(
        "class ManufacturingContainer:\n    name = 'manufacturing'\n",
        encoding="utf-8",
    )
    manifest = {
        "id": "manufacturing",
        "container": {"class": "app.containers.manufacturing.ManufacturingContainer"},
        "blocks": ["pdf", "ocr", "chat", "image", "manufacturing_v2"],
        "artifacts": [
            {"src": "app/containers/manufacturing.py", "dest": "app/containers/manufacturing.py"},
            {"src": "app/blocks/manufacturing_v2.py", "dest": "app/blocks/manufacturing_v2.py"},
        ],
    }
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic manufacturing chain."""
    return MANUFACTURING_CHAIN


def test_manufacturing_domain_smoke_contract(fake_manufacturing_engine: Path, monkeypatch, tmp_path: Path):
    """Full manufacturing concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create a manufacturing session
    session = create_session(session_id="sess_manufacturing_smoke", user_id="test_user")
    session.config.domain = "manufacturing"

    # 2. Submit the manufacturing workflow prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in MANUFACTURING_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build a manufacturing quality assurance platform."},
        )
    ...
    assert "manufacturing_v2" in block_ids
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/containers/manufacturing.py" in names
        assert "app/blocks/manufacturing_v2.py" in names
