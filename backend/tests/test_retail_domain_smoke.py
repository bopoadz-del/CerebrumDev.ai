...
It patches the chain generator to return a deterministic retail chain and then
...
# Deterministic LLM response for the retail analysis concept.
RETAIL_CHAIN: Dict[str, Any] = {
    "message": "Here is a retail analysis chain.",
    "chain": {
        "blocks": [
            {"id": "document_engine", "params": {"action": "upload_retail_docs"}},
    ...
    "rules": [
        "Always flag inventory shrinkage",
        "Always flag margin compression",
        "Always flag supplier compliance gaps",
        "Always flag fraud risk indicators",
    ],
}
...
def fake_retail_engine(tmp_path: Path, monkeypatch) -> Path:
    ...
    kit_root = engine_root / "block_store" / "kits" / "retail"
    bundle = kit_root / "bundle"

    # Kit artifacts
    (bundle / "app" / "blocks").mkdir(parents=True)
    (bundle / "app" / "blocks" / "retail_v2.py").write_text(
        "class RetailBlockV2:\n    name = 'retail_v2'\n",
        encoding="utf-8",
    )
    (bundle / "app" / "containers").mkdir(parents=True)
    (bundle / "app" / "containers" / "retail.py").write_text(
        "class RetailContainer:\n    name = 'retail'\n",
        encoding="utf-8",
    )
    manifest = {
        "id": "retail",
        "container": {"class": "app.containers.retail.RetailContainer"},
        "blocks": ["pdf", "ocr", "chat", "image", "retail_v2"],
        "artifacts": [
            {"src": "app/containers/retail.py", "dest": "app/containers/retail.py"},
            {"src": "app/blocks/retail_v2.py", "dest": "app/blocks/retail_v2.py"},
        ],
    }
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic retail chain."""
    return RETAIL_CHAIN


def test_retail_domain_smoke_contract(fake_retail_engine: Path, monkeypatch, tmp_path: Path):
    """Full retail concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create a retail session
    session = create_session(session_id="sess_retail_smoke", user_id="test_user")
    session.config.domain = "retail"

    # 2. Submit the retail workflow prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in RETAIL_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build a retail document analysis platform."},
        )
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/containers/retail.py" in names
        assert "app/blocks/retail_v2.py" in names
