...
It patches the chain generator to return a deterministic hotel_management chain and then
...
# Deterministic LLM response for the hotel_management analysis concept.
HOTEL_MANAGEMENT_CHAIN: Dict[str, Any] = {
    "message": "Here is a hotel management analysis chain.",
    "chain": {
        "blocks": [
    ...
    "rules": [
        "Always flag occupancy rate anomalies",
        "Always flag revenue leakage",
        "Always flag overbooking risk",
        "Always flag compliance violations",
        "Always flag fraud risk",
    ],
}
...
def fake_hotel_management_engine(tmp_path: Path, monkeypatch) -> Path:
    ...
    kit_root = engine_root / "block_store" / "kits" / "hotel_management"
    bundle = kit_root / "bundle"

    # Kit artifacts
    (bundle / "app" / "blocks").mkdir(parents=True)
    (bundle / "app" / "blocks" / "hotel_v2.py").write_text(
        "class HotelBlockV2:\n    name = 'hotel_management_v2'\n",
        encoding="utf-8",
    )
    (bundle / "app" / "containers").mkdir(parents=True)
    (bundle / "app" / "containers" / "hotel_management.py").write_text(
        "class HotelManagementContainer:\n    name = 'hotel_management'\n",
        encoding="utf-8",
    )
    manifest = {
        "id": "hotel_management",
        "container": {"class": "app.containers.hotel_management.HotelManagementContainer"},
        "blocks": ["pdf", "ocr", "chat", "image", "hotel_management_v2"],
        "artifacts": [
            {"src": "app/containers/hotel_management.py", "dest": "app/containers/hotel_management.py"},
            {"src": "app/blocks/hotel_v2.py", "dest": "app/blocks/hotel_v2.py"},
        ],
    }
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic hotel_management chain."""
    return HOTEL_MANAGEMENT_CHAIN


def test_hotel_management_domain_smoke_contract(fake_hotel_management_engine: Path, monkeypatch, tmp_path: Path):
    """Full hotel_management concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create a hotel_management session
    session = create_session(session_id="sess_hotel_management_smoke", user_id="test_user")
    session.config.domain = "hotel_management"

    # 2. Submit the hotel_management workflow prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in HOTEL_MANAGEMENT_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build a hotel management document analysis platform."},
        )
    ...
    assert "hotel_management_v2" in block_ids
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/containers/hotel_management.py" in names
        assert "app/blocks/hotel_v2.py" in names
