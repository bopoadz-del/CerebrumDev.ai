...
It patches the chain generator to return a deterministic construction chain and then
...
# Deterministic LLM response for the construction project-review concept.
CONSTRUCTION_CHAIN: Dict[str, Any] = {
    "message": "Here is a construction project review chain.",
    "chain": {
        "blocks": [
            {"id": "document_engine", "params": {"action": "upload_specs"}},
    ...
    "rules": [
        "Always flag schedule risk",
        "Always flag budget overruns",
        "Always flag permit compliance gaps",
        "Always flag safety violations",
    ],
}
...
def fake_construction_engine(tmp_path: Path, monkeypatch) -> Path:
    ...
    kit_root = engine_root / "block_store" / "kits" / "construction"
    bundle = kit_root / "bundle"

    # Kit artifacts
    (bundle / "app" / "blocks").mkdir(parents=True)
    (bundle / "app" / "blocks" / "construction_v2.py").write_text(
        "class ConstructionBlockV2:\n    name = 'construction_v2'\n",
        encoding="utf-8",
    )
    (bundle / "app" / "containers").mkdir(parents=True)
    (bundle / "app" / "containers" / "construction.py").write_text(
        "class ConstructionContainer:\n    name = 'construction'\n",
        encoding="utf-8",
    )
    manifest = {
        "id": "construction",
        "container": {"class": "app.containers.construction.ConstructionContainer"},
        "blocks": ["pdf", "ocr", "chat", "image", "construction_v2"],
        "artifacts": [
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic construction chain."""
    return CONSTRUCTION_CHAIN


def test_construction_domain_smoke_contract(fake_construction_engine: Path, monkeypatch, tmp_path: Path):
    """Full construction concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create a construction session
    session = create_session(session_id="sess_construction_smoke", user_id="test_user")
    session.config.domain = "construction"

    # 2. Submit the construction workflow prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in CONSTRUCTION_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build a construction project review platform."},
        )
    ...
    assert "construction_v2" in block_ids
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/containers/construction.py" in names
        assert "app/blocks/construction_v2.py" in names
