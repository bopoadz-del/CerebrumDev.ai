...
It patches the chain generator to return a deterministic pharma chain and then
...
# Deterministic LLM response for the pharma analysis concept.
PHARMA_CHAIN: Dict[str, Any] = {
    ...
    "rules": [
        "Always flag off-label usage",
        "Always flag adverse event signals",
        "Always flag compliance deviations",
        "Always flag expiration or lot tracking issues",
    ],
}
...
def fake_pharma_engine(tmp_path: Path, monkeypatch) -> Path:
    ...
    kit_root = engine_root / "block_store" / "kits" / "pharma"
    bundle = kit_root / "bundle"

    # Kit artifacts
    (bundle / "app" / "blocks").mkdir(parents=True)
    (bundle / "app" / "blocks" / "pharma_v2.py").write_text(
        "class PharmaBlockV2:\n    name = 'pharma_v2'\n",
        encoding="utf-8",
    )
    (bundle / "app" / "containers").mkdir(parents=True)
    (bundle / "app" / "containers" / "pharma.py").write_text(
        "class PharmaContainer:\n    name = 'pharma'\n",
        encoding="utf-8",
    )
    manifest = {
        "id": "pharma",
        "container": {"class": "app.containers.pharma.PharmaContainer"},
        "blocks": ["pdf", "ocr", "chat", "image", "pharma_v2"],
        "artifacts": [
            {"src": "app/containers/pharma.py", "dest": "app/containers/pharma.py"},
            {"src": "app/blocks/pharma_v2.py", "dest": "app/blocks/pharma_v2.py"},
        ],
    }
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic pharma chain."""
    return PHARMA_CHAIN


def test_pharma_domain_smoke_contract(fake_pharma_engine: Path, monkeypatch, tmp_path: Path):
    """Full pharma concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create a pharma session
    session = create_session(session_id="sess_pharma_smoke", user_id="test_user")
    session.config.domain = "pharma"

    # 2. Submit the pharma workflow prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in PHARMA_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build a pharmaceutical document analysis platform."},
        )
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/containers/pharma.py" in names
        assert "app/blocks/pharma_v2.py" in names
