...
It patches the chain generator to return a deterministic finance chain and then
...
def fake_finance_engine(tmp_path: Path, monkeypatch) -> Path:
    ...
    kit_root = engine_root / "block_store" / "kits" / "finance"
    bundle = kit_root / "bundle"

    # Kit artifacts
    (bundle / "app" / "blocks").mkdir(parents=True)
    (bundle / "app" / "blocks" / "finance_v2.py").write_text(
        "class FinanceBlockV2:\n    name = 'finance_v2'\n",
        encoding="utf-8",
    )
    (bundle / "app" / "containers").mkdir(parents=True)
    (bundle / "app" / "containers" / "finance.py").write_text(
        "class FinanceContainer:\n    name = 'finance'\n",
        encoding="utf-8",
    )
    manifest = {
        "id": "finance",
        "container": {"class": "app.containers.finance.FinanceContainer"},
        "blocks": ["pdf", "ocr", "chat", "image", "finance_v2"],
        "artifacts": [
            {"src": "app/containers/finance.py", "dest": "app/containers/finance.py"},
            {"src": "app/blocks/finance_v2.py", "dest": "app/blocks/finance_v2.py"},
        ],
    }
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic finance chain."""
    return FINANCE_CHAIN


def test_finance_domain_smoke_contract(fake_finance_engine: Path, monkeypatch, tmp_path: Path):
    """Full finance concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create a finance session
    session = create_session(session_id="sess_finance_smoke", user_id="test_user")
    session.config.domain = "finance"

    # 2. Submit the finance workflow prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in FINANCE_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build a financial document analysis platform."},
        )
    ...
    assert "finance_v2" in block_ids
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/containers/finance.py" in names
        assert "app/blocks/finance_v2.py" in names
