...
It patches the chain generator to return a deterministic education chain and then
...
# Deterministic LLM response for the education learning-assistant concept.
EDUCATION_CHAIN: Dict[str, Any] = {
    "message": "Here is an education learning-assistant chain.",
    "chain": {
        "blocks": [
            {"id": "document_engine", "params": {"action": "upload_materials"}},
            {"id": "vector_search", "params": {"action": "retrieve_context"}},
            {"id": "education_v2", "params": {"action": "tutor"}},
    ...
    "rules": [
        ...
        "Always scaffold complex concepts",
        "Always flag knowledge gaps",
    ],
}
...
def fake_education_engine(tmp_path: Path, monkeypatch) -> Path:
    """Create a minimal fake Cerebrum-Blocks engine checkout with an education kit."""
    engine_root = tmp_path / "Cerebrum-Blocks"
    kit_root = engine_root / "block_store" / "kits" / "education"
    bundle = kit_root / "bundle"

    # Kit artifacts
    (bundle / "app" / "blocks").mkdir(parents=True)
    (bundle / "app" / "blocks" / "education_v2.py").write_text(
        "class EducationBlockV2:\n    name = 'education_v2'\n",
        encoding="utf-8",
    )
    (bundle / "app" / "containers").mkdir(parents=True)
    (bundle / "app" / "containers" / "education.py").write_text(
        "class EducationContainer:\n    name = 'education'\n",
        encoding="utf-8",
    )
    manifest = {
        "id": "education",
        "container": {"class": "app.containers.education.EducationContainer"},
        "blocks": ["pdf", "ocr", "chat", "image", "education_v2"],
        "artifacts": [
            {"src": "app/containers/education.py", "dest": "app/containers/education.py"},
            {"src": "app/blocks/education_v2.py", "dest": "app/blocks/education_v2.py"},
        ],
    }
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic education chain."""
    return EDUCATION_CHAIN


def test_education_domain_smoke_contract(fake_education_engine: Path, monkeypatch, tmp_path: Path):
    """Full education concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create an education session
    session = create_session(session_id="sess_education_smoke", user_id="test_user")
    session.config.domain = "education"

    # 2. Submit the education workflow prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in EDUCATION_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build an AI tutoring platform for students."},
        )
    ...
    assert "education_v2" in block_ids
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/containers/education.py" in names
        assert "app/blocks/education_v2.py" in names
