...
It patches the chain generator to return a deterministic hr chain and then
...
# Deterministic LLM response for the hr document analysis concept.
HR_CHAIN: Dict[str, Any] = {
    "message": "Here is an HR document analysis chain.",
    "chain": {
        "blocks": [
    ...
    "rules": [
        "Always flag missing offer terms",
        "Always flag termination clauses",
        "Always flag EEO compliance gaps",
        "Always flag FLSA overtime issues",
    ],
}
...
def fake_hr_engine(tmp_path: Path, monkeypatch) -> Path:
    """Create a minimal fake Cerebrum-Blocks engine checkout with an hr kit."""
    engine_root = tmp_path / "Cerebrum-Blocks"
    kit_root = engine_root / "block_store" / "kits" / "hr"
    bundle = kit_root / "bundle"

    # Kit artifacts
    (bundle / "app" / "blocks").mkdir(parents=True)
    (bundle / "app" / "blocks" / "hr_v2.py").write_text(
        "class HRBlockV2:\n    name = 'hr_v2'\n",
        encoding="utf-8",
    )
    (bundle / "app" / "containers").mkdir(parents=True)
    (bundle / "app" / "containers" / "hr.py").write_text(
        "class HRContainer:\n    name = 'hr'\n",
        encoding="utf-8",
    )
    manifest = {
        "id": "hr",
        "container": {"class": "app.containers.hr.HRContainer"},
        "blocks": ["pdf", "ocr", "chat", "image", "hr_v2"],
        "artifacts": [
            {"src": "app/containers/hr.py", "dest": "app/containers/hr.py"},
            {"src": "app/blocks/hr_v2.py", "dest": "app/blocks/hr_v2.py"},
        ],
    }
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic hr chain."""
    return HR_CHAIN


def test_hr_domain_smoke_contract(fake_hr_engine: Path, monkeypatch, tmp_path: Path):
    """Full hr concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create an hr session
    session = create_session(session_id="sess_hr_smoke", user_id="test_user")
    session.config.domain = "hr"

    # 2. Submit the hr workflow prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in HR_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build an HR document analysis platform."},
        )
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/containers/hr.py" in names
        assert "app/blocks/hr_v2.py" in names
