...
It patches the chain generator to return a deterministic insurance chain and then
...
# Deterministic LLM response for the insurance claim-analysis concept.
INSURANCE_CHAIN: Dict[str, Any] = {
    "message": "Here is an insurance claim analysis chain.",
    "chain": {
        "blocks": [
            {"id": "document_engine", "params": {"action": "upload_claim"}},
    ...
    "rules": [
        "Always flag coverage exclusions",
        "Always flag policy limit breaches",
        "Always flag missing adjuster notes",
        "Always flag duplicate claim indicators",
    ],
}
...
def fake_insurance_engine(tmp_path: Path, monkeypatch) -> Path:
    """Create a minimal fake Cerebrum-Blocks engine checkout with an insurance kit."""
    engine_root = tmp_path / "Cerebrum-Blocks"
    kit_root = engine_root / "block_store" / "kits" / "insurance"
    bundle = kit_root / "bundle"

    # Kit artifacts
    (bundle / "app" / "blocks").mkdir(parents=True)
    (bundle / "app" / "blocks" / "insurance_v2.py").write_text(
        "class InsuranceBlockV2:\n    name = 'insurance_v2'\n",
        encoding="utf-8",
    )
    (bundle / "app" / "containers").mkdir(parents=True)
    (bundle / "app" / "containers" / "insurance.py").write_text(
        "class InsuranceContainer:\n    name = 'insurance'\n",
        encoding="utf-8",
    )
    manifest = {
        "id": "insurance",
        "container": {"class": "app.containers.insurance.InsuranceContainer"},
        "blocks": ["pdf", "ocr", "chat", "image", "insurance_v2"],
        "artifacts": [
            {"src": "app/containers/insurance.py", "dest": "app/containers/insurance.py"},
            {"src": "app/blocks/insurance_v2.py", "dest": "app/blocks/insurance_v2.py"},
        ],
    }
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic insurance chain."""
    return INSURANCE_CHAIN


def test_insurance_domain_smoke_contract(fake_insurance_engine: Path, monkeypatch, tmp_path: Path):
    """Full insurance concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create an insurance session
    session = create_session(session_id="sess_insurance_smoke", user_id="test_user")
    session.config.domain = "insurance"

    # 2. Submit the insurance workflow prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in INSURANCE_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build an insurance claim analysis platform."},
        )
    ...
    assert "insurance_v2" in block_ids
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/containers/insurance.py" in names
        assert "app/blocks/insurance_v2.py" in names
