...
It patches the chain generator to return a deterministic agriculture chain and then
...
# Deterministic LLM response for the agriculture analysis concept.
AGRICULTURE_CHAIN: Dict[str, Any] = {
    "message": "Here is an agriculture analysis chain.",
    "chain": {
        "blocks": [
            {"id": "document_engine", "params": {"action": "upload_field_reports"}},
    ...
    "rules": [
        "Always flag soil health risks",
        "Always flag irrigation inefficiencies",
        "Always flag pest or disease pressure",
        "Always flag compliance gaps",
    ],
}
...
def fake_agriculture_engine(tmp_path: Path, monkeypatch) -> Path:
    """Create a minimal fake Cerebrum-Blocks engine checkout with an agriculture kit."""
    engine_root = tmp_path / "Cerebrum-Blocks"
    kit_root = engine_root / "block_store" / "kits" / "agriculture"
    bundle = kit_root / "bundle"

    # Kit artifacts
    (bundle / "app" / "blocks").mkdir(parents=True)
    (bundle / "app" / "blocks" / "agriculture_v2.py").write_text(
        "class AgricultureBlockV2:\n    name = 'agriculture_v2'\n",
        encoding="utf-8",
    )
    (bundle / "app" / "containers").mkdir(parents=True)
    (bundle / "app" / "containers" / "agriculture.py").write_text(
        "class AgricultureContainer:\n    name = 'agriculture'\n",
        encoding="utf-8",
    )
    manifest = {
        "id": "agriculture",
        "container": {"class": "app.containers.agriculture.AgricultureContainer"},
        "blocks": ["pdf", "ocr", "chat", "image", "agriculture_v2"],
        "artifacts": [
            {"src": "app/containers/agriculture.py", "dest": "app/containers/agriculture.py"},
            {"src": "app/blocks/agriculture_v2.py", "dest": "app/blocks/agriculture_v2.py"},
        ],
    }
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic agriculture chain."""
    return AGRICULTURE_CHAIN


def test_agriculture_domain_smoke_contract(fake_agriculture_engine: Path, monkeypatch, tmp_path: Path):
    """Full agriculture concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create an agriculture session
    session = create_session(session_id="sess_agriculture_smoke", user_id="test_user")
    session.config.domain = "agriculture"

    # 2. Submit the agriculture workflow prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in AGRICULTURE_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build an agriculture field analysis platform."},
        )
    ...
    assert "agriculture_v2" in block_ids
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/containers/agriculture.py" in names
        assert "app/blocks/agriculture_v2.py" in names
