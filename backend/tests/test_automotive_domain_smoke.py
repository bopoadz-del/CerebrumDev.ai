...
It patches the chain generator to return a deterministic automotive chain and then
...
# Deterministic LLM response for the automotive document analysis concept.
AUTOMOTIVE_CHAIN: Dict[str, Any] = {
    "message": "Here is an automotive document analysis chain.",
    "chain": {
        "blocks": [
            {"id": "document_engine", "params": {"action": "upload_vehicle_docs"}},
    ...
    "rules": [
        "Always flag odometer discrepancies",
        "Always flag outstanding recalls",
        "Always flag salvage or rebuilt titles",
        "Always flag warranty expiration dates",
    ],
}
...
def fake_automotive_engine(tmp_path: Path, monkeypatch) -> Path:
    """Create a minimal fake Cerebrum-Blocks engine checkout with an automotive kit."""
    engine_root = tmp_path / "Cerebrum-Blocks"
    kit_root = engine_root / "block_store" / "kits" / "automotive"
    bundle = kit_root / "bundle"

    # Kit artifacts
    (bundle / "app" / "blocks").mkdir(parents=True)
    (bundle / "app" / "blocks" / "automotive_v2.py").write_text(
        "class AutomotiveBlockV2:\n    name = 'automotive_v2'\n",
        encoding="utf-8",
    )
    (bundle / "app" / "containers").mkdir(parents=True)
    (bundle / "app" / "containers" / "automotive.py").write_text(
        "class AutomotiveContainer:\n    name = 'automotive'\n",
        encoding="utf-8",
    )
    manifest = {
        "id": "automotive",
        "container": {"class": "app.containers.automotive.AutomotiveContainer"},
        "blocks": ["pdf", "ocr", "chat", "image", "automotive_v2"],
        "artifacts": [
            {"src": "app/containers/automotive.py", "dest": "app/containers/automotive.py"},
            {"src": "app/blocks/automotive_v2.py", "dest": "app/blocks/automotive_v2.py"},
        ],
    }
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic automotive chain."""
    return AUTOMOTIVE_CHAIN


def test_automotive_domain_smoke_contract(fake_automotive_engine: Path, monkeypatch, tmp_path: Path):
    """Full automotive concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create an automotive session
    session = create_session(session_id="sess_automotive_smoke", user_id="test_user")
    session.config.domain = "automotive"

    # 2. Submit the automotive workflow prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in AUTOMOTIVE_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build an automotive document analysis platform."},
        )
    ...
    assert "automotive_v2" in block_ids
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/containers/automotive.py" in names
        assert "app/blocks/automotive_v2.py" in names
