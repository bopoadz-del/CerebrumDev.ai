...
It patches the chain generator to return a deterministic medical chain and then
...
# Deterministic LLM response for the medical analysis concept.
MEDICAL_CHAIN: Dict[str, Any] = {
    "message": "Here is a medical analysis chain.",
    "chain": {
        "blocks": [
            {"id": "document_engine", "params": {"action": "upload_records"}},
    ...
    "rules": [
        "Always flag critical lab values",
        "Always flag medication interactions",
        "Always flag allergy conflicts",
        "Always flag missing clinical history",
    ],
}
...
def fake_medical_engine(tmp_path: Path, monkeypatch) -> Path:
    ...
    (bundle / "app" / "blocks" / "medical_v2.py").write_text(
        "class MedicalBlockV2:\n    name = 'medical_v2'\n",
        encoding="utf-8",
    )
    ...
    manifest = {
        ...
        "blocks": ["pdf", "ocr", "chat", "image", "medical_v2"],
        "artifacts": [
            {"src": "app/containers/medical.py", "dest": "app/containers/medical.py"},
            {"src": "app/blocks/medical_v2.py", "dest": "app/blocks/medical_v2.py"},
        ],
    }
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic medical chain."""
    return MEDICAL_CHAIN


def test_medical_domain_smoke_contract(fake_medical_engine: Path, monkeypatch, tmp_path: Path):
    """Full medical concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create a medical session
    session = create_session(session_id="sess_medical_smoke", user_id="test_user")
    session.config.domain = "medical"

    # 2. Submit the medical workflow prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in MEDICAL_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build a medical document analysis platform."},
        )
    ...
    assert "medical_v2" in block_ids
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/blocks/medical_v2.py" in names
