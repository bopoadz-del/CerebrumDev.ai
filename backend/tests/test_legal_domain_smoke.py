...
It patches the chain generator to return a deterministic legal chain and then
...
def fake_legal_engine(tmp_path: Path, monkeypatch) -> Path:
    ...
    kit_root = engine_root / "block_store" / "kits" / "legal"
    bundle = kit_root / "bundle"

    # Kit artifacts
    (bundle / "app" / "blocks").mkdir(parents=True)
    (bundle / "app" / "blocks" / "legal_v2.py").write_text(
        "class LegalBlockV2:\n    name = 'legal_v2'\n",
        encoding="utf-8",
    )
    (bundle / "app" / "containers").mkdir(parents=True)
    (bundle / "app" / "containers" / "legal.py").write_text(
        "class LegalContainer:\n    name = 'legal'\n",
        encoding="utf-8",
    )
    manifest = {
        "id": "legal",
        "container": {"class": "app.containers.legal.LegalContainer"},
        "blocks": ["pdf", "ocr", "chat", "image", "legal_v2"],
        "artifacts": [
            {"src": "app/containers/legal.py", "dest": "app/containers/legal.py"},
            {"src": "app/blocks/legal_v2.py", "dest": "app/blocks/legal_v2.py"},
        ],
    }
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic legal chain."""
    return LEGAL_CHAIN


def test_legal_domain_smoke_contract(fake_legal_engine: Path, monkeypatch, tmp_path: Path):
    """Full legal concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create a legal session
    session = create_session(session_id="sess_legal_smoke", user_id="test_user")
    session.config.domain = "legal"
    # patch update/get if needed; session_store uses in-memory dict by default

    # 2. Submit the contract-review prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in LEGAL_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build a legal contract review platform."},
        )
    ...
    assert "legal_v2" in block_ids
    assert "chat" in block_ids
    assert block_ids & {"document_engine", "pdf"}
    assert block_ids & {"ocr", "ocr_v2"}
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/containers/legal.py" in names
        assert "app/blocks/legal_v2.py" in names
