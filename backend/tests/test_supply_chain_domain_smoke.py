"""Domain smoke contract test for the supply chain concept flow.
...
It patches the chain generator to return a deterministic supply chain chain and then
...
# Deterministic LLM response for the supply chain analysis concept.
SUPPLY_CHAIN_CHAIN: Dict[str, Any] = {
    "message": "Here is a supply chain analysis chain.",
    "chain": {
        "blocks": [
            {"id": "document_engine", "params": {"action": "upload_shipment_docs"}},
    ...
    "rules": [
        "Always flag supplier risk",
        "Always flag shipment delays",
        "Always flag inventory anomalies",
        "Always flag compliance violations",
    ],
}
...
def fake_supply_chain_engine(tmp_path: Path, monkeypatch) -> Path:
    """Create a minimal fake Cerebrum-Blocks engine checkout with a supply chain kit."""
    engine_root = tmp_path / "Cerebrum-Blocks"
    kit_root = engine_root / "block_store" / "kits" / "supply_chain"
    bundle = kit_root / "bundle"

    # Kit artifacts
    (bundle / "app" / "blocks").mkdir(parents=True)
    (bundle / "app" / "blocks" / "supply_chain_v2.py").write_text(
        "class SupplyChainBlockV2:\n    name = 'supply_chain_v2'\n",
        encoding="utf-8",
    )
    (bundle / "app" / "containers").mkdir(parents=True)
    (bundle / "app" / "containers" / "supply_chain.py").write_text(
        "class SupplyChainContainer:\n    name = 'supply_chain'\n",
        encoding="utf-8",
    )
    manifest = {
        "id": "supply_chain",
        "container": {"class": "app.containers.supply_chain.SupplyChainContainer"},
        "blocks": ["pdf", "ocr", "chat", "image", "supply_chain_v2"],
        "artifacts": [
            {"src": "app/containers/supply_chain.py", "dest": "app/containers/supply_chain.py"},
            {"src": "app/blocks/supply_chain_v2.py", "dest": "app/blocks/supply_chain_v2.py"},
        ],
    }
...
async def _mock_generate_chain_suggestion(*args, **kwargs) -> Dict[str, Any]:
    """Return the deterministic supply chain chain."""
    return SUPPLY_CHAIN_CHAIN


def test_supply_chain_domain_smoke_contract(fake_supply_chain_engine: Path, monkeypatch, tmp_path: Path):
    """Full supply chain concept flow: chat -> preview -> approve -> package -> inspect."""
    monkeypatch.setenv("STORAGE_PATH", str(tmp_path / "storage"))

    # 1. Create a supply chain session
    session = create_session(session_id="sess_supply_chain_smoke", user_id="test_user")
    session.config.domain = "supply_chain"

    # 2. Submit the supply chain workflow prompt with patched chain generator
    #    and block registry (so no real Cerebrum-Blocks backend is needed).
    fake_registry = {b["id"]: {"name": b["id"]} for b in SUPPLY_CHAIN_CHAIN["chain"]["blocks"]}
    with patch(
        ...
        response = client.post(
            ...
            json={"message": "I want to build a supply chain document analysis platform."},
        )
    ...
    assert "supply_chain_v2" in block_ids
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as zf:
        ...
        assert "app/containers/supply_chain.py" in names
        assert "app/blocks/supply_chain_v2.py" in names
