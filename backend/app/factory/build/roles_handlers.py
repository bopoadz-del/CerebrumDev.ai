...
def _compiled_writer_brief(ctx: RoleContext) -> str:
    """The BRIEF the worker must follow.

    Priority: the user's own words from the Floor chat (threaded from the
    session as ``state['brief']``), then the frozen compiled C-BRIEF
    (block contracts, REUSE inventory, gap list) CLONER wrote to
    docs/coder_brief.md, then the blueprint's capability specs.
    ...
    caps = getattr(ctx.blueprint, "capabilities", None) or []
    lines = [summary] if summary else []
    for cap in caps:
        cid = str(getattr(cap, "id", "") or "").strip()
        desc = str(getattr(cap, "description", "") or "").strip()
        if cid:
            lines.append(f"- {cid}: {desc}".rstrip())
    return "\n".join(lines)
