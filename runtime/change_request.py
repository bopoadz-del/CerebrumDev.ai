...
def load(cr_path: str) -> ChangeRequest:
    ...
    if not p.is_file():
        stop.halt(
            "cr_missing", {"path": cr_path}, "Provide the approved change-request path."
        )
    try:
    ...
    if not isinstance(sig, dict) or not all(
        sig.get(k) for k in ("algorithm", "public_key_id", "value")
    ):
        stop.halt(
            "cr_unsigned", {}, "Only approved (signed) change-requests run — sign at intake."
        )
    return ChangeRequest(
        ...
        dna_version=str(doc["dna_version"]),
    )
